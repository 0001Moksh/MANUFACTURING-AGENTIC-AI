...
const SUGGESTED_PROMPTS = [
  ...
  'Any active hard hat or safety vest violations?',
  'List violations detected during night shifts',
  'Which plant has the highest number of safety violations?',
];
...
export const PPEVisionAgentPage: React.FC = () => {
  ...
  const threadId = useMemo(() => `ppe-${Date.now()}`, []);
  ...
  const startNewConversation = () => {
    setMessages([{ id: `welcome-${Date.now()}`, role: 'assistant', text: 'New conversation started. Ask about PPE compliance, CCTV incidents, zone breaches, or shift analytics.' }]);
    setInput('');
    setActiveSteps([]);
  };
      ...
      <div className="bg-gradient-to-r from-[#0B1730] via-[#162746] to-[#C2410C] text-white rounded-[20px] p-6 relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          <div>
            ...
            <div className="flex gap-2 mt-4 flex-wrap text-[11px] text-white/80">
              {[
                ...
                'Hazard Zone Geofencing',
                'Shift Analytics',
                'Unremediated Flags',
                'HSE Rule Engine',
                'Worker Movement Audits',
              ].map((tag) => (
          ...
          <div className="flex gap-3 shrink-0 flex-wrap">
            {[
              { icon: <Camera className="w-4 h-4 text-amber-300" />, label: 'Tools', value: '55 Vision Tools' },
              { icon: <AlertTriangle className="w-4 h-4 text-amber-300" />, label: 'Inspection', value: 'PostgreSQL (RO)' },
              { icon: <Flame className="w-4 h-4 text-amber-300" />, label: 'Gateway', value: 'Gemini / Groq' },
              { icon: <HardHat className="w-4 h-4 text-amber-300" />, label: 'Persona', value: 'Deva by Moksh' },
            ].map((chip) => (
              <div
                key={chip.label}
        ...
        <div className="border-t border-border-color bg-white">
            ...
            <div className="px-5 pt-3 pb-1 flex flex-wrap gap-2">
              {SUGGESTED_PROMPTS.map((p) => (
                <button
                  key={p}
                  onClick={() => void send(p)}
                  className="text-[11.5px] px-3 py-1.5 rounded-full border border-amber-400 text-amber-800 hover:bg-amber-50 transition-colors bg-transparent cursor-pointer font-medium"
                >
                  {p}
                </button>
              ))}
          ...
          <div className="flex gap-3 items-end p-4">
            <textarea
              ...
              className="flex-1 resize-none rounded-[14px] border border-border-color bg-canvas px-4 py-3 text-[14px] outline-none focus:border-amber-600 transition-colors"
              disabled={loading}
            />
            <button
              id="ppe-agent-send"
              ...
              className="inline-flex items-center gap-2 rounded-[14px] bg-gradient-to-br from-[#0B1730] to-[#C2410C] px-4 py-3 text-white font-semibold disabled:opacity-50 transition-opacity hover:opacity-90 shrink-0"
            >
              {loading ? (
