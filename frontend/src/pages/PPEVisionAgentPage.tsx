...
import { ppeVisionService } from '../services/ppeVisionService';
...
function inlineMarkdown(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (/^`[^`]+`$/.test(part)) {
      return (
        <code key={i} className="bg-amber-50 text-amber-800 rounded px-1 text-[12px] font-mono border border-amber-200">
          {part.slice(1, -1)}
        </code>
      );
    }
...
function MessageBubble({
  ...
  return (
    ...
    >
      {!isUser && (
        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shrink-0 mr-2 mt-1 shadow-xs">
...
export const PPEVisionAgentPage: React.FC = () => {
  const navigate = useNavigate();
  const [threadId, setThreadId] = useState<string>(() => `ppe-${Date.now()}`);
  ...
  const startNewConversation = () => {
    setThreadId(`ppe-${Date.now()}`);
    setMessages([
      {
        id: `welcome-${Date.now()}`,
        role: 'assistant',
        text: 'New conversation started. Ask about PPE compliance, CCTV incidents, zone breaches, or shift analytics.',
        timestamp: 'Just now',
      },
    ]);
    setInput('');
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pt-2 shrink-0 px-3">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center pt-2 gap-2 text-[13px] font-semibold text-muted hover:text-amber-600 transition-colors bg-transparent border-none cursor-pointer p-0"
        >
      ...
      <div className="bg-gradient-to-r from-[#0B1730] via-[#162746] to-[#C2410C] text-white rounded-[16px] shadow-sm shrink-0 border border-navy-700/50 overflow-hidden mx-3">
            ...
            >
              <div className="px-4 pb-4 pt-1 border-t border-white/10">
                <p className="text-white/80 text-[13px] max-w-[840px] mt-2 mb-0 leading-relaxed">
                  Computer vision intelligence across CCTV streams detecting hard-hats, high-visibility vests, restricted geofence breaches, worker fatigue, and shift anomalies with 55 real-time query tools.
                </p>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
                  <div className="bg-white/10 backdrop-blur-xs rounded-[12px] px-3 py-2 border border-white/10 text-center">
                    <div className="flex justify-center mb-1"><Camera className="w-3.5 h-3.5 text-amber-300" /></div>
                    <div className="text-[13px] font-extrabold text-white">55 Tools</div>
                    <div className="text-[9.5px] text-white/70 font-semibold uppercase">Vision Tools</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-xs rounded-[12px] px-3 py-2 border border-white/10 text-center">
                    <div className="flex justify-center mb-1"><AlertTriangle className="w-3.5 h-3.5 text-amber-300" /></div>
                    <div className="text-[13px] font-extrabold text-white">PostgreSQL</div>
                    <div className="text-[9.5px] text-white/70 font-semibold uppercase">Inspection (RO)</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-xs rounded-[12px] px-3 py-2 border border-white/10 text-center">
                    <div className="flex justify-center mb-1"><Flame className="w-3.5 h-3.5 text-amber-300" /></div>
                    <div className="text-[13px] font-extrabold text-white">Gemini/Groq</div>
                    <div className="text-[9.5px] text-white/70 font-semibold uppercase">Gateway</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-xs rounded-[12px] px-3 py-2 border border-white/10 text-center">
                    <div className="flex justify-center mb-1"><HardHat className="w-3.5 h-3.5 text-amber-300" /></div>
        ...
        <div className="border-t border-border-color bg-white p-2.5 md:p-3 space-y-2 shrink-0">
          ...
          <div className="flex gap-2 items-end">
            <textarea
              ...
              rows={1}
              placeholder="Ask Deva about helmet/vest compliance, zone breaches, CCTV logs, shifts..."
              className="flex-1 resize-none rounded-[12px] border border-border-color bg-canvas px-3.5 py-2 text-[13.5px] outline-none focus:border-amber-600 focus:ring-2 focus:ring-amber-500/20 transition-all placeholder:text-muted max-h-[140px]"
              disabled={loading}
            />
            <button
              id="ppe-agent-send"
              ...
              className="inline-flex items-center gap-2 rounded-[12px] bg-gradient-to-br from-[#0B1730] to-[#C2410C] px-4 py-2.5 text-white font-semibold text-[13.5px] shadow-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer border-none active:scale-95"
            >
...
export default PPEVisionAgentPage;