...
/* ------------------------------------------------------------------ */
/* Types & helpers (shared with RoleDetailPage / PermissionBuilder)   */
/* ------------------------------------------------------------------ */
...
const PAD_TOP = 24;
const COL = {
  root: { left: 16, width: 140 },
  module: { left: 220, width: 168 },
  resource: { left: 452, width: 190 },
  action: { left: 706, width: 150 },
};
...
const PermissionFlowDiagram: React.FC<PermissionFlowDiagramProps> = ({ userLabel, rules }) => {
        ...
        >
          ...
          {Array.from(moduleRanges.entries()).map(([moduleKey, m]) => {
            ...
            return (
              <div
                key={moduleKey}
                className="absolute flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[12px] font-bold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
          ...
          {Array.from(resourceRanges.values()).map((r) => {
            ...
            return (
              <div
                ...
                className="absolute flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11.5px] font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
...
export const UserDetailPage: React.FC = () => {
  ...
  // Parse permissions into structured rules
  const rules: BuilderPermissionRule[] = useMemo(() => {
    ...
    if (parsed.length === 0 && Array.isArray(user.permissions)) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            ...
            className="rounded-[10px] border border-border-color bg-white p-2 text-ink cursor-pointer hover:bg-panel"
          >
          ...
          <div>
            ...
            <h2 className="font-head text-[28px] font-extrabold text-ink mt-1">{displayName}</h2>
          </div>
        </div>
        <button
          type="button"
          onClick={() => navigate(`/admin/users/${id}/edit`)}
          className="rounded-[8px] border border-border-color bg-white px-3 py-2 text-[12px] font-bold text-ink cursor-pointer hover:bg-panel"
        >
          Edit
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div className="bg-panel border border-border-color rounded-[14px] p-4">
          <div className="flex items-center gap-2 mb-3 text-[11px] font-bold text-muted uppercase tracking-[0.5px]">
            <User className="h-3.5 w-3.5" />
            Username
          </div>
          <div className="text-[15px] font-bold text-ink truncate">{user.username}</div>
        </div>
        ...
        <div className="bg-panel border border-border-color rounded-[14px] p-4">
          ...
          <div className="text-[15px] font-bold text-ink truncate">{user.role}</div>
        </div>
        ...
        <div className="bg-panel border border-border-color rounded-[14px] p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold text-ink truncate">
              {user.scope?.value || user.site || 'Platform'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="text-[10.5px] text-muted mt-0.5">
            ...
            {user.role ? ` • Inherited from role “${user.role}”` : ''}
          </div>
        </div>
      ...
      {rules.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-10 text-center">
          ...
          <div className="mt-1 text-[12px] text-slate-400 dark:text-slate-500">
            This user may only have access coming from their assigned role.
          </div>
        </div>
      ) : viewMode === 'map' ? (
        <PermissionFlowDiagram userLabel={displayName} rules={rules} />
      ) : (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {groupedRules.map(([moduleKey, group]) => {
            ...
            return (
              <div
                key={moduleKey}
                className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/50"
              >
                  ...
                  {group.rows.map((rule) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-white/60 p-2.5 dark:bg-slate-950/30"
                      >
