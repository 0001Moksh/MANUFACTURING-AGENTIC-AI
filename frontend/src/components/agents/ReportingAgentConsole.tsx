...
export const AgentChatConsole: React.FC = () => {
  ...
  const handleQuery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReportingAgentState({ status: 'error', errorMsg: err.response?.data?.detail || 'An error occurred during workflow execution.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setReportingAgentState({ status: 'error', errorMsg: err.response?.data?.detail || 'Failed to execute database write action.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-panel border border-border-color rounded-[14px] p-6 mt-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        {status !== 'idle' && (
          <button type="button" onClick={resetReportingAgentState} className="flex items-center gap-1.5 text-[11.5px] font-semibold text-muted hover:text-red transition-colors bg-surface hover:bg-red-tint/30 border border-border-color rounded-lg px-2.5 py-1 cursor-pointer">
            <RotateCcw className="w-3.5 h-3.5" /> Reset State
          </button>
        )}
      </div>
      <p className="text-[12.5px] text-muted mb-4">Ask Deva to pull telemetry or query orders.</p>
      <form onSubmit={handleQuery} className="flex gap-2 mb-5">
        <div className="flex-1 relative">
          <input type="text" placeholder="Type your query to database..." value={query} onChange={(e) => setReportingAgentState({ query: e.target.value })} disabled={loading} className="w-full bg-[#FAFBFE] border border-border-color rounded-[9px] p-[10px_14px] text-[13px] text-ink focus:outline-none focus:border-teal/50" />
        </div>
        <div className="bg-white border border-border-color rounded-[9px] p-[10px] text-[12px] font-semibold text-muted flex items-center">Model: Auto</div>
        <button type="submit" disabled={loading} className="bg-teal text-white border-none rounded-[9px] p-[10px_18px] text-[12px] font-bold cursor-pointer hover:bg-teal-deep transition-colors flex items-center gap-1 shrink-0">
          {loading ? <RefreshCw className="animate-spin w-[14px] h-[14px]" /> : <Send className="w-[14px] h-[14px]" />} Send
        </button>
      </form>

      {status !== 'idle' && explainableLogs && (
        <div className="mb-4">
          <button type="button" onClick={() => setReportingAgentState({ showLogs: !showLogs })} className="text-[11.5px] font-semibold text-teal hover:text-teal-deep flex items-center gap-1 cursor-pointer bg-transparent border-none py-1 focus:outline-none">
          ...
          {showLogs && (
            <div className="bg-[#101423] text-[#A6ACCD] font-mono text-[11px] rounded-[10px] p-4 mt-2 select-none max-h-[220px] overflow-y-auto">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2"><Cpu className="text-[#C792EA] w-[14px] h-[14px]" /><span>Initializing AgentState dictionary...</span></div>
                {result?.execution_steps?.map((step: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 pl-3"><CheckCircle className="text-[#C3E88D] w-[12px] h-[12px]" /><span>{step}</span></div>
                ))}
              </div>
            </div>
          )}
      ...
      {status === 'error' && errorMsg && (
        <div className="bg-red-tint border border-red text-red rounded-[10px] p-4 flex gap-3 items-start mb-4">
          ...
          <div><div className="font-bold text-[13.5px]">Access Denied / Query Failed</div><div className="text-[12.5px] mt-0.5 leading-relaxed">{errorMsg}</div></div>
        </div>
      )}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center border-b border-border-color pb-3">
            ...
            {!(result.error_message || result.insights?.includes('Unable to complete database search')) && result.pdf_url && (
              <button onClick={() => handleDownloadPdf(result.pdf_url)} className="bg-teal hover:bg-teal-deep text-white border-none rounded-[8px] p-[6px_14px] text-[11.5px] font-bold cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm"><FileDown className="w-[14px] h-[14px]" />Download PDF Report</button>
            )}
          </div>
