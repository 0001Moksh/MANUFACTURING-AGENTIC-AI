...
export const VideoMonitoringChatWidget: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Floating Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-22 right-6 z-50 w-[92vw] sm:w-[480px] h-[640px] max-h-[85vh] bg-white border border-slate-200 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
          <div className="px-4 py-3 bg-white border-b border-slate-200 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-slate-500 truncate">Video Monitoring & Safety Intelligence</p>
              </div>
            </div>
                ...
                <div className={`w-full max-w-[92%] ${msg.sender === 'user' ? 'items-end ml-auto' : 'items-start'}`}>
                  ...
                  >
                    {msg.sender === 'user' ? (
                      ...
                      <div className="prose prose-slate prose-sm max-w-none text-slate-800 text-[13px] leading-relaxed break-words">
                        <ReactMarkdown
                          remarkPlugins={[remarkGfm as any]}
                          components={{
                            table: ({ node, ...props }) => (
                              <div className="my-2.5 overflow-x-auto rounded-lg border border-slate-200 shadow-xs">
                                <table className="w-full text-left text-xs border-collapse bg-white" {...props} />
                              </div>
                            ),
                            thead: ({ node, ...props }) => (
                              <thead className="bg-slate-100/90 text-slate-700 font-semibold border-b border-slate-200 text-[11px] uppercase tracking-wider" {...props} />
                            ),
                            th: ({ node, ...props }) => (
                              <th className="py-2 px-2.5 font-bold text-slate-700" {...props} />
                            ),
                            td: ({ node, ...props }) => {
                              ...
                              if (isOnline || isCompliant) {
                                return (
                                  <td className="py-2 px-2.5 border-t border-slate-100 text-slate-700">
                                    <span className="inline-flex items-center gap-1 font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded text-[11px] border border-emerald-200">
                              ...
                              if (isOffline || isCritical) {
                                return (
                                  <td className="py-2 px-2.5 border-t border-slate-100 text-slate-700">
                                    <span className="inline-flex items-center gap-1 font-semibold text-red-700 bg-red-50 px-1.5 py-0.5 rounded text-[11px] border border-red-200">
                                      {props.children}
                                    </span>
                                  </td>
                                );
                              ...
                              if (isWarning) {
                                return (
                                  <td className="py-2 px-2.5 border-t border-slate-100 text-slate-700">
                                    <span className="inline-flex items-center gap-1 font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded text-[11px] border border-amber-200">
                                      {props.children}
                                    </span>
                                  </td>
                                );
                              }
                              return <td className="py-2 px-2.5 border-t border-slate-100 text-slate-700" {...props} />;
                            },
                            h1: ({ node, ...props }) => (
                              <h1 className="text-[15px] font-bold text-slate-900 mt-2 mb-1.5 tracking-tight" {...props} />
                            ),
                            h2: ({ node, ...props }) => (
                              <h2 className="text-[14px] font-bold text-slate-900 mt-2 mb-1 tracking-tight" {...props} />
                            ),
                            h3: ({ node, ...props }) => (
                              <h3 className="text-[13px] font-bold text-slate-900 mt-1.5 mb-1" {...props} />
                            ),
                            h4: ({ node, ...props }) => (
                              <h4 className="text-[12px] font-bold text-slate-800 mt-1 mb-0.5" {...props} />
                            ),
                            ul: ({ node, ...props }) => (
                              <ul className="list-disc pl-4 space-y-1 my-1.5 text-slate-700" {...props} />
                            ),
                            ol: ({ node, ...props }) => (
                              <ol className="list-decimal pl-4 space-y-1 my-1.5 text-slate-700" {...props} />
                            ),
                            blockquote: ({ node, ...props }) => (
                              <blockquote className="border-l-3 border-teal-500 bg-teal-50/60 pl-2.5 py-1 my-1.5 text-slate-700 italic rounded-r text-[12px]" {...props} />
                            ),
                            code: ({ node, inline, ...props }: any) =>
                              inline ? (
                                <code className="bg-slate-100 text-slate-800 px-1 py-0.5 rounded font-mono text-[11px] border border-slate-200" {...props} />
                              ) : (
                                <code className="block bg-slate-900 text-slate-100 p-2.5 rounded-lg font-mono text-[11px] my-2 overflow-x-auto" {...props} />
                              ),
                          }}
                        >
                          {msg.text}
                      ...
                      <span className="inline-flex items-center gap-2 text-slate-500 text-xs py-1">
                        <span className="font-medium text-slate-600">Analyzing safety telemetry</span>
          ...
          <div className="px-3 py-1.5 bg-slate-100/60 border-t border-slate-200/60 flex items-center gap-1.5 overflow-x-auto text-[11px] shrink-0 no-scrollbar">
            ...
            <button
              onClick={() => handleSendMessage('Show live feed for CAM-02')}
              disabled={isStreaming}
              className="px-2 py-0.5 bg-white border border-slate-200 hover:border-teal-400 hover:text-teal-700 rounded-full text-slate-600 shrink-0 transition-colors cursor-pointer"
            >
              Live Feed
          ...
          <div className="p-3 bg-white border-t border-slate-200 flex items-center gap-2 shrink-0">
            <input
              ...
              disabled={isStreaming}
              className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 text-[13px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition disabled:opacity-60"
            />
            <button
              onClick={() => handleSendMessage()}
              disabled={isStreaming || !input.trim()}
              className="p-2 bg-teal-600 hover:bg-teal-700 disabled:bg-slate-200 text-white disabled:text-slate-400 rounded-lg transition-all disabled:opacity-60 cursor-pointer shadow-sm"
              title="Send Query"
            >
              ...
              <div className="space-y-5 text-[13px]">
                ...
                <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                  ...
                  <button
                    ...
                    className={`w-10 h-5.5 rounded-full p-0.5 transition-colors shrink-0 cursor-pointer ${
                      governance.requireHitlForMutations ? 'bg-teal-600' : 'bg-slate-300'
                    }`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${
                        governance.requireHitlForMutations ? 'translate-x-4.5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg">
                  ...
                  <button
                    ...
                    className={`w-10 h-5.5 rounded-full p-0.5 transition-colors shrink-0 cursor-pointer ${
                      governance.alertAudioEnabled ? 'bg-teal-600' : 'bg-slate-300'
                    }`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${
                        governance.alertAudioEnabled ? 'translate-x-4.5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </div>
