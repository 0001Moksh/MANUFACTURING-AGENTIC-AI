"""
Production configuration for Voice AI System.
...
class VADConfig:
    ...
    min_silence_frames: int = int(os.getenv("VAD_MIN_SILENCE_FRAMES", "8"))  # Increased from 5
...
class TTSConfig:
    ...
    cache_ttl_seconds: int = int(os.getenv("TTS_CACHE_TTL", str(3600)))  # 1 hour
...
class RouterConfig:
    ...
    # Use ML-based router if available; fallback to keyword matching
...
class MetricsConfig:
    ...
    # Prometheus endpoint, Datadog, etc.
...
class Config:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "llm": {k: v for k, v in self.llm.__dict__.items() if k != "groq_api_key"},  # Don't log API key
