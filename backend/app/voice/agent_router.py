...
Routes spoken natural language queries to the appropriate LangGraph agent
(Safety & Quality, PPE Vision, Maintenance, General) and returns ground-truth
data for LLM summarization.

print() replaced with structured logging for production observability.
"""
...
def detect_agent_intent(prompt: str) -> str:
    ...
    Falls back to the general workflow agent if the intent is ambiguous.
    """
    lower = prompt.lower()

    # PPE & CCTV Vision Agent
    ppe_keywords = [
...
async def execute_agent_query(
    ...
    """
    Execute a query on the target LangGraph agent (or auto-route it).

    Returns:
        dict with keys: agent_id, agent_name, reply (ground-truth insights)
    """
    chosen_agent = (
        agent_id if (agent_id and agent_id != "auto")
        else detect_agent_intent(prompt)
    )

    logger.info(
        "Routing voice query to agent",
        chosen_agent=chosen_agent,
        query_preview=prompt[:60],
    )
