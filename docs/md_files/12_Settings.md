...
## Purpose
Document the platform settings panel and its enterprise controls.

## Settings
| Setting | Meaning |
|---|---|
| White-label branding | Custom logo, colors, and domain |
| SSO / SAML / Azure AD | Identity federation across roles and plants |
| API keys & webhooks | Programmatic access to modules |
| Data retention | Retention windows for time-series, video, and audit data |
| Environment | Sandbox to staging to production promotion |

## Relationships
- Identity and access are detailed in [04_User_Roles](./04_User_Roles.md).
- Governance and audit controls are detailed in [10_Admin_Console](./10_Admin_Console.md).
